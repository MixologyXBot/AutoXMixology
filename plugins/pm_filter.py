...
async def next_page(bot, query):
    ...
    #     ]
        ...
        btn.insert(0, 
            [
                InlineKeyboardButton(f'Select ⬇️', 'select')
            ]
        )
        btn.insert(0, [
            InlineKeyboardButton("Start Bot ✔️", url=f"https://telegram.me/{temp.U_NAME}"),
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("Page 📖", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("Next ➡️", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("Page 📖", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton("Next ➡️", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("⬅️ Back", callback_data=f"next_{req}_{key}_{off_set}"),
    ...
    if not settings["button"]:
        if settings["imdb"]:
            ...
            cap = f"<b>Hey {query.from_user.mention}, \nFound {total} Results For Your Query: {search}\n\n</b><b>Note:</b> <spoiler>This Message Will Be Deleted After 5 Minutes To Prevent Copyright !</spoiler>\n\n"
            cap+="<b><u>🗂️ Select Files Here ⬇️</u></b>\n\n"
            for file in files:
                cap += f"<b>📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'>{' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.') and not x.startswith('tg') and not x.startswith('telegram'), file.file_name.split()))} [{get_size(file.file_size)}]\n\n</a></b>"
        try:
...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    if offset != "":
        try:
            if settings['max_btn']:
                btn.append(
                    [InlineKeyboardButton("Page 📖", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="Next ➡️",callback_data=f"next_{req}_{key}_{offset}")]
                )
    
            else:
                btn.append(
                    [InlineKeyboardButton("𝐏𝐀𝐆ᴇ", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="Next ➡️",callback_data=f"next_{req}_{key}_{offset}")]
                )
        except KeyError:
            await save_group_settings(query.message.chat.id, 'max_btn', True)
            btn.append(
                [InlineKeyboardButton("𝐏𝐀𝐆ᴇ", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="Next ➡️",callback_data=f"next_{req}_{key}_{offset}")]
            )
    else:
        btn.append(
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("You need to join @moviemixologymanager Channel to use this bot!", show_alert=True)
            return
        ...
        if settings is not None:
            buttons = [
                ...
                [
                    ...
                    InlineKeyboardButton('🔄 Manual Start' if settings["botpm"] else '☑️ Auto Mode',
                                         callback_data=f'setgs#botpm#{settings["botpm"]}#{str(grp_id)}')
                ],
                [
                    InlineKeyboardButton('🛡️ Protect Content',
                ...
                [
                    InlineKeyboardButton('✔️ Spell Check',
                ...
                [
                    InlineKeyboardButton('🎛️ Auto Filter',
        ...
        if query.from_user.id in ADMINS:
            ...
            try:
                await client.send_message(chat_id=int(from_user), text=f"Hey {user.mention}, \nSorry Your Request is Unavailable. So Our Moderators Can't Upload it.", reply_markup=InlineKeyboardMarkup(btn2))
            except UserIsBlocked:
                await client.send_message(chat_id=int(SUPPORT_CHAT_ID), text=f"Hey {user.mention}, \nSorry Your Request is Unavailable. So Our Moderators Can't Upload it.\n\n<b>Note:</b> <spoiler>This Message is Sent To This Group Because You've Blocked The Bot.</spoiler>", reply_markup=InlineKeyboardMarkup(btn2))
        else:
            await query.answer("Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢʜᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
        ...
        if query.from_user.id in ADMINS:
            ...
            try:
                await client.send_message(chat_id=int(from_user), text=f"Hey {user.mention}, \nYour Request Has Been Uploaded By Our Moderators. 𝖪𝗂𝗇𝖽𝗅𝗒 S𝖾𝖺𝗋𝖼𝗁 in Our Group <b>@MixologyMoviesSeries</b> !", reply_markup=InlineKeyboardMarkup(btn2))
            except UserIsBlocked:
                await client.send_message(chat_id=int(SUPPORT_CHAT_ID), text=f"Hey {user.mention}, \nYour Request Has Been Uploaded By Our Moderators. Kindly Search in Our Group <b>@MixologyMoviesSeries</b> !\n\n<b>Note:</b> <spoiler>This Message is Sent To This Group Because You've Blocked The Bot. To Get Your File(s), Must Unblock The Bot.</spoiler>", reply_markup=InlineKeyboardMarkup(btn2))
        else:
        ...
        await query.message.edit_text(
            ...
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
                InputMediaPhoto("https://telegra.ph/file/0e80926da4585c4296cf6.jpg")
            )
            reply_markup = InlineKeyboardMarkup(btn)
            await query.message.edit_text(
                text="👀"
            ...
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
                InputMediaPhoto("https://telegra.ph/file/3d5c9d42f5d5ba472b376.jpg")
            )
            reply_markup = InlineKeyboardMarkup(btn)
            await query.message.edit_text(
                text="▣☐☐"
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            dlt = await message.reply_sticker('CAACAgUAAxkBAAEKvO1lUMTjJ4zcMtpcnqSuNGC3NXq1rQACxAIAAgRtoFbBdTYPZ_QqxzME', reply_markup=InlineKeyboardMarkup(btn))
            search = search.lower()
            find = search.split(" ")
            search = ""
    ...
    #     btn.insert(0, [
    #         InlineKeyboardButton("Start Bot ✔️", url=f"https://telegram.me/{temp.U_NAME}"),
    #         InlineKeyboardButton("Send All Files 📂", url=await get_shortlink(message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=allfiles_{key}")
    #         )
    #     ])
    # else:
    #     btn.insert(0, [
    #         InlineKeyboardButton("Start Bot ✔️", url=f"https://telegram.me/{temp.U_NAME}"),
    #         InlineKeyboardButton("Send All Files 📂", callback_data=f"send_fall#{pre}#{message.chat.id}-{message.id}#{0}")
    #     ])
        btn.insert(0, [
            InlineKeyboardButton("Start Bot ✔️", url=f"https://telegram.me/{temp.U_NAME}"),
            InlineKeyboardButton("Send All Files 📂", callback_data=f"sendfiles#{key}")
    ...
    if offset != "":
        ...
        try:
            if settings['max_btn']:
                ...
                btn.append(
                    [InlineKeyboardButton("Page 📖", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="Next ➡️",callback_data=f"next_{req}_{key}_{offset}")]
                )
        except KeyError:
